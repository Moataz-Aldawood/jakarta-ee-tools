...
export class JsfDocumentLinkProvider implements vscode.DocumentLinkProvider {
    public provideDocumentLinks(
        ...
        while ((tagMatch = tagRegex.exec(docText)) !== null) {
            ...
            if (activeCatalogs[fullTagName]) {
                ...
                const tagLink = new vscode.DocumentLink(
                    new vscode.Range(nameStartPos, nameEndPos),
                    vscode.Uri.parse(docUrl)
                );
                ...
                while ((attrMatch = attrRegex.exec(attrsText)) !== null) {
                    ...
                    const attrLink = new vscode.DocumentLink(
                        new vscode.Range(attrStartPos, attrEndPos),
                        vscode.Uri.parse(docUrl + "#" + attrName)
                    );
